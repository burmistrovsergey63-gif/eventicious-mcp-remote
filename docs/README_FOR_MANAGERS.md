...
## Правила наполнения курса контентом

`courseId` недостаточно для наполнения курса. Нужен полный набор ID из ledger.

### Какие ID нужны

| Тип контента | Нужный ID | Откуда берётся |
|---|---|---|
| Text 2.0 | `stageCatalogId` | `stages[].catalogId` из ответа импорта курса |
| PassTest / PassPoll | `pollId` | `stages[].pollId` из ответа импорта курса |
| Task | `taskContentId` | `stages[].taskContentId` из ответа импорта курса |

### Порядок операций

1. Загрузить обложку курса
2. Импортировать полную структуру курса
3. Сохранить raw response и обновить `EVENTICIOUS_MCP_IDS.md`
4. Импортировать Task content (пока курс в draft)
5. Импортировать Poll/Test content через `pollId`
6. Добавить Text 2.0 через `stageCatalogId`
7. Проверить готовность курса
8. Финализировать курс

### Ограничения

- Task content нужно импортировать до финализации. Если курс уже финализирован, а Task content не был загружен — сообщите о блокере.
- У Text 2.0 нет update-инструмента. Для изменения текста нужно удалить старый и создать новый (меняется `catalogElementId`).
- Структура этапов (тип,条件, порядок) и настройки курса (название, описание, прогресс, дедлайн) задаются до создания курса.
- Инструментов `eventicious_update_course` и `eventicious_unfinalize_course` пока нет.

### Текущие gaps (известные dev gaps)

- Нет `eventicious_update_text2` — обновление Text 2.0
- Нет `eventicious_update_course` — обновление настроек курса
- Нет `eventicious_unfinalize_course` — возврат курса в draft
